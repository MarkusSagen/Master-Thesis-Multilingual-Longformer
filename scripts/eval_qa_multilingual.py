...
def get_squad_evaluation(valid_dataset, model, tokenizer, dataset_name, batch_size):
    ...
    """
    # TODO check for is format == torch
    # print("Format: ", valid_dataset.format['type'])
    ...
    logging.info("Evaluation on this dataset completed... \n\n" + "=" * 55 + "\n\n" * 3)
...
def main():
    ...
    # FIXME: Move repeated data transormations in here
    def question_answer_pairs_to_tensor(dict_of_datasets):
        """
        "name of dataset":  the-loaded-dataset
        """
        pass



    # Configure and set up the data correctly
    # FIXME: Move them to main!
    # TODO: Create a list or dict with all datasets a user enters, then load them and for each dataset, make evaluation
    # FIXME: load in easy way
    # TODO: Move the other datasets to the end, since starting the training is only dependent on a single train and eval dataset
    # DONT REPEAT YOUR SELF STUPID!
    columns = ['input_ids', 'attention_mask', 'start_positions', 'end_positions']

    # FIXME: This check will break when moving to lists of datasets
    if data_args.datasets == "xquad":
        ...
        xquad_de = nlp.load_dataset('xquad', 'xquad.de', split="validation")

        valid_dataset_en = xquad_en.map(convert_to_features).filter(lambda example: (example['start_positions'] is not None) and (example['end_positions'] is not None))
        valid_dataset_ru = xquad_ru.map(convert_to_features).filter(lambda example: (example['start_positions'] is not None) and (example['end_positions'] is not None))
        valid_dataset_ar = xquad_ar.map(convert_to_features).filter(lambda example: (example['start_positions'] is not None) and (example['end_positions'] is not None))
        valid_dataset_de = xquad_de.map(convert_to_features).filter(lambda example: (example['start_positions'] is not None) and (example['end_positions'] is not None))

        valid_dataset_en.set_format(type='torch', columns=columns)
        valid_dataset_ru.set_format(type='torch', columns=columns)
        valid_dataset_ar.set_format(type='torch', columns=columns)
        valid_dataset_de.set_format(type='torch', columns=columns)
    ...
    # TODO make better
    if data_args.datasets == "xquad"  or  data_args.datasets == "squad":
    ...
    #get_squad_evaluation(valid_dataset_en, model, tokenizer, "XQuAD English validation", training_args.per_device_eval_batch_size)
    #get_squad_evaluation(valid_dataset_ru, model, tokenizer, "XQuAD Russian validation", training_args.per_device_eval_batch_size)
    #get_squad_evaluation(valid_dataset_ar, model, tokenizer, "XQuAD Arabic validation", training_args.per_device_eval_batch_size)
    #get_squad_evaluation(valid_dataset_de, model, tokenizer, "XQuAD German validation", training_args.per_device_eval_batch_size)
